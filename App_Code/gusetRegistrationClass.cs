...
public class gusetRegistrationClass
{
    ...
    public static guestBookingAttributes[] getlocalClienthistor(int bid)
    {
        ctownDataContext db = new ctownDataContext();
        var result = (from b in db.bookings
                      join g in db.guests on b.guest_id equals g.Id
                      ...
                      join r in db.booking_Rooms on b.Id equals r.bookingId
                      join room in db.rooms on r.roomid equals room.Id
                      where g.guest_type == "local" && b.branch_id == bid
                      select new
                      {
                          booking = b,
                          guest = g,
    ...
    public static guestBookingAttributes[] getforignerClienthistory(int bid)
    {
        ctownDataContext db = new ctownDataContext();
        var result = (from b in db.bookings
                      join g in db.guests on b.guest_id equals g.Id
                      join r in db.booking_Rooms on b.Id equals r.bookingId
                      join room in db.rooms on r.roomid equals room.Id
                      where g.guest_type == "forigner" && b.branch_id == bid
                      select new
                      {
                          booking = b,
                          guest = g,
