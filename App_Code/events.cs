...
public class events
{
    ...
    public static bool addEvent(event_calender ev)
    {
        try
        {
            ...
            int count = (from x in db.event_calenders
                         where x.event_description== ev.event_description && x.event_color == ev.event_color && x.event_start_date == ev.event_start_date && x.employee_id== ev.employee_id
                         select x).Count();
            if (count == 0)
            {
    ...
    public static bool updateEvent(event_calender ec, int id)
    {
        ...
        updatedEvent.employee_id = ec.employee_id;
